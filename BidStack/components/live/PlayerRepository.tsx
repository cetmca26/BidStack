...
export function PlayerRepository({ players, teams, onBack }: PlayerRepositoryProps) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-900/40 rounded-2xl border border-slate-800 overflow-hidden" data-density="compact">
            {/* Header */}
            <div className="flex flex-col gap-3 p-4 sm:p-5 border-b border-slate-800 flex-shrink-0">
                ...
                <div className="relative">
                    ...
                    <Input
                        ...
                        className="pl-9 bg-slate-950/50 border-slate-800 text-sm h-9"
                    />
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                    {filteredPlayers.map((player) => {
                        ...
                        return (
                            <div
                                key={player.id}
                                className="p-2.5 sm:p-3 rounded-xl border border-slate-800 bg-slate-950/30 flex items-center gap-2.5 sm:gap-3 group hover:border-emerald-500/30 transition-all"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-[8px] font-black text-slate-500 uppercase tracking-wider">
                                        {player.role}
                                    </div>
                                    {player.status === "sold" && team ? (
                                        <div className="mt-1 flex items-center gap-1 min-w-0">
                                            <TeamLogo name={team.name} logoUrl={team.logo_url} size="sm" />
                                            <div className="text-[8px] font-black text-emerald-400 uppercase truncate">
                                                {team.name}
                                            </div>
                                        </div>
                                    ) : (
                                        <div
                                            className={`mt-1 text-[8px] font-black uppercase tracking-widest ${player.status === "live" ? "text-amber-500 animate-pulse" : "text-slate-600"}`}
                                        >
                                            {player.status}
                                        </div>
                                    )}
                                ...
                                {player.status === "sold" && (
                                    <div className="text-right flex-shrink-0 text-[9px] sm:text-xs font-mono font-bold text-slate-300">
                ...
                {filteredPlayers.length === 0 && (
                    <div className="py-12 text-center text-slate-500 italic uppercase font-bold tracking-widest text-sm">
